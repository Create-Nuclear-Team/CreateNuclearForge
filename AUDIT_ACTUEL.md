...
**Décompte global** *(mis à jour 2026-07-14, commit `9e8abe92`)* : **2 fichiers sur 292** (~0,7 %) contiennent encore du français détectable, pour environ **4 lignes** de commentaires concernées. **Aucun symbole de code** (nom de classe, méthode, champ) en français n'a été trouvé — le problème est **localisé aux commentaires**, pas à l'API elle-même.
...
**Pattern récurrent** : le point « commentaires FR/EN mêlés » déjà signalé dans `AUDIT_V1.md` §5 (sans jamais être suivi comme item séparé) **est désormais quasiment clos** : il ne reste que `NuclearExplosionEntity.java` (logique non triviale) et `CNEntityType.java` (cosmétique, nouvellement trouvé). Tous les mixins, `CNFluids`, `ClientEvents`, le rendu Display Link, les palettes de décoration et le worldgen irradié sont désormais entièrement en anglais. Le sous-ensemble concerné est passé de 11/292 à 2/292 fichiers (~3,8 % → ~0,7 %) en un seul commit (`9e8abe92`) — aucun nouveau commentaire français n'a été introduit entre-temps par les commits fonctionnels (JEI, worldgen, tests).

**Pourquoi ça doit être suivi** : le dernier cas non trivial restant (`NuclearExplosionEntity.java`) porte l'explication du *pourquoi* d'un choix non trivial (compat cross-mod) — c'est-à-dire exactement le contenu qu'un commentaire doit porter selon les conventions du projet. Le laisser uniquement en français limite la relecture/contribution à l'équipe francophone actuelle et complique tout audit ou onboarding futur non francophone (y compris par un outil ou un contributeur externe à la communauté Create).

**Priorité recommandée** : 🟢 **cosmétique, à faire au fil de l'eau — pas bloquant.** Aucun symbole d'API n'est en français, donc pas d'impact sur la lisibilité des signatures pour un contributeur externe. Les 4 cas de Javadoc public (`VicinityEffect.java` et `CNPonderReactorScenes.java` ×3) et les 9 fichiers de commentaires inline du tour précédent (`3be6252a`, `9e8abe92`) sont traduits et committés. Il ne reste que 2 fichiers, ~4 lignes : `NuclearExplosionEntity.java:45,50` (à traduire en priorité, logique non triviale) et `CNEntityType.java:76` (cosmétique). **Suivi** : cet item reste en §6, à re-scanner périodiquement — proche de la clôture complète.
...
- Reste : `content/explosion/NuclearExplosionEntity.java:45,50` (compat Alex's Caves, logique non triviale) et `CNEntityType.java:76` (cosmétique, nouvellement trouvé) — voir §6. **Justification** : cosmétique et non bloquant (aucun symbole d'API en français), mais le premier explique un « pourquoi » non trivial (compat cross-mod) — le laisser en français limite l'audit/onboarding futur.
